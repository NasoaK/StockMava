...
function exportData($categorie){
    ...
    $fileName = "Stock_articles_" .$categorie . date('Y-m-d') . ".xls"; 
    
    // Column names 
    $fields = array('nom','quantity'); 
    
    // Display column names as first row 
    $excelData = implode("\t", array_values($fields)) . "\n"; 
    
    // Fetch records from database 
    $ventes= $db->query("SELECT nom, quantity FROM ArticlesMava WHERE `categorie` = '$categorie' "); 
    
    if($ventes->num_rows > 0){ 
        // Output each row of the data 
        while($row = $ventes->fetch_assoc()){ 
            $lineData = array(str_replace("\"","",iconv('UTF-8','ASCII//TRANSLIT',$row['nom'])), $row['quantity']); 
